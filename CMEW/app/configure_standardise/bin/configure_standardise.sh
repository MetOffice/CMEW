...
# For Test model
cmew-process-env create_request_file.py --path "${REQUEST_PATH}" --model_id "${MODEL_ID}" --suite_id "${SUITE_ID}" --calendar "${CALENDAR}" --variant "${VARIANT_LABEL}"

# For Reference model
cmew-process-env create_request_file.py --path "${REQUEST_PATH_REFERENCE}" --model_id "${MODEL_ID_REFERENCE}" --suite_id "${SUITE_ID_REFERENCE}" --calendar "${CALENDAR_REFERENCE}" --variant "${VARIANT_LABEL_REFERENCE}"
